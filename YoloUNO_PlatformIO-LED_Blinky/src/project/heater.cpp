...
void heater_task()
{
    ...
    case HEATER_GREEN:
        ...
        digitalWrite(HEATER_LED_PIN2, LOW);
        if(current_temp > THRESHOLD_LOW && current_temp < THRESHOLD_NORMAL)
        {
    ...
    case HEATER_YELLOW:
        ...
        //GREEN WHEN CURRENT TEMP < THRESHOLD_LOW
        if(current_temp <= THRESHOLD_LOW)
        {
            heater_state = HEATER_GREEN;
            Serial.println("Heater state:  GREEN");
        }
        //RED WHEN CURRENT TEMP => THRESHOLD_NORMAL
        else if (current_temp => THRESHOLD_NORMAL)
        {
            heater_state = HEATER_RED;
            Serial.println("Heater state:  RED");
    ...
    case HEATER_RED:
        ...
        else if (current_temp < THRESHOLD_NORMAL && current_temp > THRESHOLD_LOW)
        {
            heater_state = HEATER_YELLOW;
            Serial.println("Heater state:  YELLOW");
        }
